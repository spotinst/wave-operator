...
type input struct {
	sourceFolder, targetFile string
}
...
func main() {
	for _, i := range inputs {
		pack(i.sourceFolder, i.targetFile)
	}
}

func pack(embedFolder, blobFileName string) {
	// Checking directory with files
	if _, err := os.Stat(embedFolder); os.IsNotExist(err) {
		log.Fatal("Static directory " + embedFolder + " does not exist!")
	}
